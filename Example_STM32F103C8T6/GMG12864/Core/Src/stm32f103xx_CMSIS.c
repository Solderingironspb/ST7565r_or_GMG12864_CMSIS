...
void CMSIS_I2C_Reset(I2C_TypeDef* I2C) {
    // Сброс настроек I2C
...
/**
 *************************************************************************************
 *  @breif Функция инициализации шины I2C1, I2C2. Sm.
 *************************************************************************************
 */

void CMSIS_I2C_Init(I2C_TypeDef* I2C) {
    ...
    if (I2C == I2C1) {
        ...
        SET_BIT(RCC->APB1ENR, RCC_APB1ENR_I2C2EN);  // Запуск тактирования I2C2
    }
    ...
    MODIFY_REG(I2C->CR2, I2C_CR2_FREQ_Msk, 36 << I2C_CR2_FREQ_Pos);  // f PCLK1 = 36 Мгц
    ...
    CLEAR_BIT(I2C->CCR, I2C_CCR_FS);  // Standard mode I2C
    ...
    MODIFY_REG(I2C->CCR, I2C_CCR_CCR_Msk, 180 << I2C_CCR_CCR_Pos);  // для Sm mode
    // MODIFY_REG(I2C->CCR, I2C_CCR_CCR_Msk, 30 << I2C_CCR_CCR_Pos); //для Fm mode. DUTY 0.
    // MODIFY_REG(I2C->CCR, I2C_CCR_CCR_Msk, 4 << I2C_CCR_CCR_Pos); //для Fm mode. DUTY 1.

    // п.п. 26.6.9 I2C TRISE register (I2C_TRISE)(стр. 782)
    MODIFY_REG(I2C->TRISE, I2C_TRISE_TRISE_Msk, 37 << I2C_TRISE_TRISE_Pos);  // для Sm mode
    // MODIFY_REG(I2C1->TRISE, I2C_TRISE_TRISE_Msk, 12 << I2C_TRISE_TRISE_Pos); //для Fm mode

    SET_BIT(I2C->CR1, I2C_CR1_PE);  // I2C enable
}
...
void CMSIS_SPI1_init(void) {
     ...
     * 111: fPCLK/256
     * */
    MODIFY_REG(SPI1->CR1, SPI_CR1_BR, 0b000 << SPI_CR1_BR_Pos);  // fPCLK/4. 36000000/2 = 18 MBits/s
    ...
    // CS - PA6:
    MODIFY_REG(GPIOA->CRL, GPIO_CRL_MODE6, 0b11 << GPIO_CRL_MODE6_Pos);  // Maximum output speed 50 MHz
    MODIFY_REG(GPIOA->CRL, GPIO_CRL_CNF6, 0b00 << GPIO_CRL_CNF6_Pos);    // Output Push pull
    // MOSI - PA7:
...
FLASH_Page_write(0x0800F000, (uint8_t*)&Flash_data_STM, sizeof(Flash_data_STM));
FLASH_Read_data(0x0800F000, (uint8_t*)&Flash_data_STM1, sizeof(Flash_data_STM1));
*/
 ...
 *  см. п.п. 2.3.1 Key values(стр. 12)
 ***************************************************************************************
 */
 ...
 *  Programming Manual PM0075
 *  см. п. 3.5 Flash control register (FLASH_CR)(стр. 26)
 ***************************************************************************************
 */
 ...
 *  Programming Manual PM0075
 *  см. п. 2.3.4 Flash memory erase(стр. 15)
